...
* 	利用计算的宽度，把预设截面样式复制到这个点，并按宽度进行缩放，作为截面 		已完成
...
* 操作时实时显示效果，文件tab中允许设置体素大小(体素化时也用SDF来判断内外)和渲染SDF 		已完成
* 每根毛发独占一个addon_renderer 		已完成
* 毛发节点自带sdf，为每根毛发的SDF_Mesh的并集
* 附加件的碰撞编辑器不再是下拉框选择模式，而是两个勾选框：
*	显露：模型的SDF会减去底模 		已完成
*   拆分：点击更新碰撞以后，每根发束会生成独立节点，并且每一根发束还会减去所有排在它前面的发束（勾选显露以后会在之后减去底模） 		已完成
*
* 在上面的调试完成以后再做：
