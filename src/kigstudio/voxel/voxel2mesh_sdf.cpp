...
class SmoothMeshGenerator {
   ...
   private:
    ...
    vec3f voxelToWorld(int vx, int vy, int vz) const {
        return vec3f(voxelData_.global_position.x +
                         (vx + 0.5f) * voxelData_.voxel_size.x,
                     voxelData_.global_position.y +
                         (vy + 0.5f) * voxelData_.voxel_size.y,
                     voxelData_.global_position.z +
                         (vz + 0.5f) * voxelData_.voxel_size.z);
    }

    vec3f gridIndexToWorld(int cx, int cy, int cz, float gx, float gy, float gz)
        const {
        int base_vx = cx << 5;  // cx * 32
        int base_vy = cy << 5;
        int base_vz = cz << 5;
        return vec3f(
            voxelData_.global_position.x +
                (base_vx + (gx - 1.0f) / subdivisions_) *
                    voxelData_.voxel_size.x,
            voxelData_.global_position.y +
                (base_vy + (gy - 1.0f) / subdivisions_) *
                    voxelData_.voxel_size.y,
            voxelData_.global_position.z +
                (base_vz + (gz - 1.0f) / subdivisions_) *
                    voxelData_.voxel_size.z);
    }

    // 采样voxel occupancy，支持边界扩展
    bool sampleVoxelOccupancy(int cx, int cy, int cz, int lx, int ly, int lz)
        const {
    ...
    // 采样SDF值
    float sampleSDF(int cx,
                    int cy,
                    int cz,
                    int x,
                    int y,
                    int z,
                    const ChunkSDFData& chunk_sdf) const {
        // (x,y,z) 在subdivided chunk坐标系中，范围为[0, 32*subdivisions+3)
        float cached = chunk_sdf.get(x, y, z);
        if (std::isfinite(cached))
            return cached;

        vec3f wp =
            gridIndexToWorld(cx, cy, cz, static_cast<float>(x),
        ...
        if (sdf_ptr_) {
            ...
            // 线性插值占有率，转换为符号距离
            float gx = (static_cast<float>(x) - 1.0f) / subdivisions_;
            float gy = (static_cast<float>(y) - 1.0f) / subdivisions_;
            float gz = (static_cast<float>(z) - 1.0f) / subdivisions_;
    ...
    void processChunk(int cx, int cy, int cz, TriangleCallback callback) {
        ...
        // 创建当前chunk的SDF缓存
        int size = 32 * subdivisions_ + 3;  // +3用于两侧边界扩展
                ...
                for (int x = 0; x < size; ++x) {
                    ...
                    if (count % update_interval == 0) {
                        ...
                        status_callback_(
                            current_status + " " +
                            get_locale_string(
                                "progress.surface_nets.building_voxel_sdf") +
    ...
    void extractChunkVertices(int cx,
                              int cy,
                              int cz,
                              ChunkSDFData& chunk_sdf,
                              std::vector<vec3f>& chunk_positions,
        ...
        // Include one negative halo cell so quads on the chunk's lower faces can
        // reference p - axis, and include the positive edge cells owned here.
        int cube_size = 32 * subdivisions_;
        int vertex_cube_size = cube_size + 1;
        int64_t total =
            int64_t(vertex_cube_size) * vertex_cube_size * vertex_cube_size;
        int64_t count = 0;
        int64_t update_interval = std::max<int64_t>(1, total / 100);
    ...
    void buildChunkFaces(int size,
        ...
        auto maybe_make_quad = [&](int p1, int p2, int axis_b_stride,
            ...
            uint32_t v4 =
                chunk_grid_to_vertex[p1 - axis_b_stride - axis_c_stride];
