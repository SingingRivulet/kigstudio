...
// 模糊后的全局 SSAO 结果（fs_ao + fs_ao_blur 两个中间 pass 生成）
SAMPLER2D(s_ao, 7);
...
{   
    ...
    // 全局 SSAO：乘以 AO pass（fs_ao + fs_ao_blur）预计算的遮蔽系数，
    // 所有表面互相遮蔽，靠近的面变暗。
    // 必须反转y轴：AO pass 渲染到 RT，内容与屏幕上下颠倒（同 s_collision）
    lighting *= texture2D(s_ao, vec2(v_texcoord0.x, 1.0-v_texcoord0.y)).r;
